...
impl<T: Read + Seek> PacketIn<T> for StatusResponsePacket {
...
impl<T: Write + Seek> PacketOut<T> for StatusResponsePacket {
    fn write(&self, writer: &mut PacketWriter<T>) {
        writer.write_str(self.field_status.as_str());
    }
}

impl PacketRecv for StatusResponsePacket {}
impl PacketSend for StatusResponsePacket {}
