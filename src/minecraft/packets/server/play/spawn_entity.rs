...
pub struct SpawnEntityPacket {
    ...
    pitch: u8,         // To get the real pitch, you must divide this by (256.0F / 360.0F)
    yaw: u8,           // To get the real yaw, you must divide this by (256.0F / 360.0F)
    head_yaw: u8,      // Only used by living entities, where the head of the entity may differ from the general body rotation.
...
impl<T: Write + Seek> PacketOut<T> for SpawnEntityPacket {
    fn write(&self, writer: &mut PacketWriter<T>) {
        ...
        writer.write_angle(self.pitch);
        writer.write_angle(self.yaw);
        writer.write_angle(self.head_yaw);
